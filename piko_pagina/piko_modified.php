...
if (isset($_GET['api']) && $_GET['api'] === 'read') {
    ...
    if (!is_array($data)) { $data = ['actualTemp' => 20, 'cald' => 0, 'setPoint' => 20]; }
...
(function(){
  ...
  function renderState(state){
    // Temperature
    if (tempEl) {
      tempEl.textContent = "tempmis: " + (typeof state.actualTemp === 'number' ? state.actualTemp.toFixed(1) : state.actualTemp) + "°C";
    }
    // Heater status
    if (statusEl) {
